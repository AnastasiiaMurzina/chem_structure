...
class Spherical_divider():
    def __init__(self, n):
        ...
        self.scube = divs
        self.d_min_q = np.linalg.norm(divs[0] - divs[2])
        self.neighbours = {i: [] for i in range(len(self.scube))}
        self.set_neighbours()
        for i in range(len(divs)):
            self.scube[i] = self.scube[i] / np.linalg.norm(self.scube[i])
    ...
    def nearest_from_to2(self, s1, s2):
        distances = []
        for neighbour in self.neighbours[s1]:
            distances.append([np.linalg.norm(self.scube[s2]-self.scube[neighbour]), neighbour])
...
if __name__ == '__main__':
    ...
    print([len(i) for k, i in s.neighbours.items()])
