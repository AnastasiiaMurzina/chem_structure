    ...
    if with_attr:
        for key, item in one_way_bonds.items():
            ...
            for i in item:
                ...
                if two_ways.get(i[0]):
                    app = [key]
                    print('app', app.extend([i[1]]))
                    two_ways[i[0]].append(app.extend(i[1::]))
                else:
                    app = [key]
                    print('app', i[1], app.extend([i[1]]))
                    two_ways.update({i[0]: app.extend(i[1::])})
            # print(two_ways)
            # if two_ways.get(i[1]):
...
def mol2_to_notation(info_from_file):
    ...
    for key, item in atoms.items():
        ...
        connected = []
        if bonds.get(key):
            connected = [i[0] for i in bonds[key]]
        # print(connected)
            # print(i)
    #     connected = [k for k, ite in atoms.items() if abs(np.linalg.norm(ite.position() - cur_p) - 1) < valid_length]
    #     sections = []
